...
| Argument | Type | Default | What it expects |
| --- | --- | --- | --- |
| `neojunctions_file` | positional | `pvactools_output/neoantigen.intersected.filtered_epitopes.tsv` | Tab-delimited peptide table from Step 4. |
| `reference_fasta` | positional | `example/uniprotkb_Human_AND_model_organism_9606_2024_07_25_sp.fasta` | Reference protein FASTA. Plain text FASTA, gzip-compressed FASTA, and single-file zip archives are supported. |
| `--output-prefix` | optional | `output/neoantigen_reference_check` | Prefix used for the generated TSV report files. |
| `--peptide-column-name` | optional | `new_peptide` | Peptide column name in the neojunction table. |
| `--peptide-column` | optional | unset | 1-based peptide column index. Used only when `--peptide-column-name` is not provided. |

Outputs written from `--output-prefix`:

| File | Meaning |
| --- | --- |
| `{output-prefix}.unique.tsv` | One row per unique peptide, with peptide length, input row count, and `found_in_reference` (`yes`/`no`). |
| `{output-prefix}.rows.tsv` | Original input rows plus a `found_in_reference` column. |
| `{output-prefix}.matched.rows.tsv` | Only the input rows whose peptide sequence is present in the reference proteome. |

With the example command above, the script writes:

- `output/neoantigen_reference_check.unique.tsv`
- `output/neoantigen_reference_check.rows.tsv`
- `output/neoantigen_reference_check.matched.rows.tsv`
